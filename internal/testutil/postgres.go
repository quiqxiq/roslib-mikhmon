...
// test expiry service.
func NewStores(t *testing.T) (store.DeviceStore, store.ProfileConfigStore, store.TransactionStore) {
	t.Helper()
	db := NewPostgres(t)
	return store.NewDeviceStore(db), store.NewProfileConfigStore(db), store.NewTransactionStore(db)
}
